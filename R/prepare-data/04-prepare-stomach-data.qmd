...
d <- #read_csv("https://raw.githubusercontent.com/maxlindmark/pred-prey-overlap/main/data/clean/catch_clean.csv") |> 
  read_csv(paste0(home, "/data/stomach/stomachs3.csv")) |> 
  filter(year >= 1993) |> 
  dplyr::select(fr_tot, fr_sad, fr_spr, fr_her, year, month, day, day_of_year, pred_weight,
                pred_length, lat, lon, ices_rect, X, Y, data_source, Country) |> 
  filter(fr_tot > 0 & fr_tot < quantile(fr_tot, .99))
```
...
density_saduria <- terra::extract(saduria_latlon, env_dat |> dplyr::select(lon, lat))

env_dat$saduria <- density_saduria$FWBiomassm_raster_19812019presHighweightcor_no0_newZi
```
