...
## range overlap
range_overlapspfn<-function(prey, pred){
  pred > 0 & prey > 0
}

## area overlap
area_overlapspfn<-function(prey, pred){
  pred > 0 & prey > 0
}
...
## Schoener's overlap
schoeners_overlapspfn <- function(prey, pred) {
  p_prey <- prey/sum(prey, na.rm = T)
  p_pred <- pred/sum(pred, na.rm = T)
  abs(p_prey-p_pred)
}
...
## global index of collocation, spatial (get centre of gravity and inertia axes)
...
## AB ratio
AB_overlapspfn <- function(prey, pred) { 
...
bhatta_coefspfn <- function(prey, pred) {
  p_prey <- prey/sum(prey, na.rm = T)
  p_pred <- pred/sum(pred, na.rm = T)
  sqrt(p_prey*p_pred)
}
