...
```{r predict}
#| cache: false
#mcod_pel_pred <- predict(mcod_pel, newdata = pred_grid, type = "response")
#mcod_sad_pred <- predict(mcod_sad, newdata = pred_grid, type = "response")
mcod_com_pred <- predict(mcod_com, newdata = pred_grid, type = "response")

# write_csv(mcod_pel_pred, paste0(home, "/output/mcod_pel_pred.csv"))
# write_csv(mcod_sad_pred, paste0(home, "/output/mcod_sad_pred.csv"))
write_csv(mcod_com_pred, paste0(home, "/output/pred_cod.csv"))
```
...
### TODO: Plot time-varying varying
...
## Some test plots! 
Not yet for the combined model

```{r}
mcod_pel_pred <- read_csv(paste0(home, "/output/mcod_pel_pred.csv"))
mcod_sad_pred <- read_csv(paste0(home, "/output/mcod_sad_pred.csv"))

# Pelagic cod
long_pel <- mcod_pel_pred |>
  filter(year == 1999) |> 
...
plot_map +
  geom_raster(data = long_pel, aes(X*1000, Y*1000, fill = zeta)) + 
  facet_grid(Model~Quarter) +
  scale_fill_viridis(option = "mako") + 
  labs(title = "Spatially varying effect of quarter",
       subtitle = "Cod: pelagic")

ggsave(paste0(home, "/figures/supp/density_zeta_pel_cod.pdf"), width = 17, height = 17, units = "cm")

# Saduria cod
long_sad <- mcod_sad_pred |>
  filter(year == 1999) |> 
  pivot_longer(c("zeta_s_quarter_f11", "zeta_s_quarter_f12", "zeta_s_quarter_f41", "zeta_s_quarter_f42"),
               names_to = "field", values_to = "zeta") |> 
  mutate(Quarter = ifelse(field %in% c("zeta_s_quarter_f11", "zeta_s_quarter_f12"), "Quarter 1", "Quarter 4"),
...
plot_map +
  geom_raster(data = long_sad, aes(X*1000, Y*1000, fill = zeta)) + 
  facet_grid(Model~Quarter) +
  scale_fill_viridis(option = "mako") + 
  labs(title = "Spatially varying effect of quarter",
       subtitle = "Cod: pelagic")

ggsave(paste0(home, "/figures/supp/density_zeta_sad_cod.pdf"), width = 17, height = 17, units = "cm")

# Plot correlation between predictions
cor(mcod_pel_pred$est1, mcod_sad_pred$est1)
cor(mcod_pel_pred$est2, mcod_sad_pred$est2)
```

- Probably proceeed with a single cod model... the correlation between these two size groups is 0.99...