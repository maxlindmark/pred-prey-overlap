...
ggsave(paste0(home, "/figures/supp/spatial_resid_cod_com.pdf"), width = 17, height = 17, units = "cm")

# Biomass density
plot_map_fc +
  geom_raster(data = filter(com_pred, quarter == 1), aes(X*1000, Y*1000, fill = est), size = 0.5) +
  facet_wrap(~year) + 
...
plot_map_fc +
  geom_raster(data = filter(com_pred, quarter == 4), aes(X*1000, Y*1000, fill = est), size = 0.5) +
  facet_wrap(~year) + 
  scale_fill_viridis(trans = "sqrt", name = "Biomass density (kg/km)")
...
plot_map_fc +
  ...
  scale_fill_viridis(option = "mako", name = "Zeta")
