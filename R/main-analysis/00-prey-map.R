...
sad <- ggplot(swe_coast_proj) +
  ...
  facet_wrap(~Prey) +
  geom_raster(
    data = pred_grid |> filter(Prey == "Saduria"),
  ...
  geom_text(
    data = annotations |> mutate(annotateText = c("(b)")), size = 3.2,
    aes(x = xpos, y = ypos, hjust = hjust, vjust = vjust, label = annotateText)
  )
...
spr <- plot_map2 +
  geom_sf(color = "gray80") +
  geom_raster(
    data = pred_grid |> filter(Prey == "Sprat"),
    aes(X * 1000, Y * 1000, fill = value / 1000)
  ) +
  scale_fill_viridis(option = "viridis", name = "Biomass\n(t tonnes)") +
  theme(
  ...
  geom_text(
    data = annotations |> mutate(annotateText = c("(c)")), size = 3.2,
    aes(x = xpos, y = ypos, hjust = hjust, vjust = vjust, label = annotateText)
  )
