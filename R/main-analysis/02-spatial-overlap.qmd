...
ggplot(data = pel_annual, aes(year, value, color = Scale, fill = Scale)) +
  geom_point(alpha = 0.5) +
  geom_line(alpha = 0.5) + 
  geom_smooth(method = "gam", formula = y~s(x, k = 4), alpha = 0.2) + 
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  labs(x = "Year", y = "Biomass-weighted overlap index") +
  facet_wrap(~ Species) +
  theme(legend.position = c(0.42, 0.12))
...
cod_ben_sum |> 
  ...
  geom_line(alpha = 0.5) + 
  geom_smooth(method = "gam", formula = y~s(x, k = 4), alpha = 0.2) + 
  labs(x = "Year", y = "Biomass-weighted overlap index", color = "", fill = "") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1")

ggsave(paste0(home, "/figures/saduria_overlap.pdf"), width = 11, height = 7, units = "cm")
```
