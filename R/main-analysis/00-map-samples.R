...
source(paste0(here::here(), "/R/functions/map-plot.R"))
...
plot_map +
  ...
  geom_contour(
    data = pred_grid, aes(X * 1000, Y * 1000,
      z = depth,
  ...
  guides(
    ...
    color = guide_colorbar(title.position = "top", title.hjust = 0.5, position = "inside"),
    size = guide_legend(title.position = "top", title.hjust = 0.5, ncol = 2, position = "inside")
  ) +
  theme(
    legend.position.inside = c(0.77, 0.09),
  ...
  annotate("text", label = "Sweden", x = xmin2 + 0.15 * xrange, y = ymin2 + 0.75 * yrange, color = "gray30", size = 2.6) +
  annotate("text", label = "Germany", x = xmin2 + 0.01 * xrange, y = ymin2 + 0.01 * yrange, color = "gray30", size = 2.6) +
  annotate("text", label = "Poland", x = xmin2 + 0.5 * xrange, y = ymin2 + 0.005 * yrange, color = "gray30", size = 2.6) +
  annotate("text", label = "Russia", x = xmin2 + 0.95 * xrange, y = ymin2 + 0.18 * yrange, color = "gray30", size = 2.6) +
  annotate("text", label = "Lithuania", x = xmin2 + 0.99 * xrange, y = ymin2 + 0.44 * yrange, color = "gray30", size = 2.6) +
  annotate("text", label = "Latvia", x = xmin2 + 0.99 * xrange, y = ymin2 + 0.65 * yrange, color = "gray30", size = 2.6) +
